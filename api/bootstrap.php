...
$stmt = $pdo->prepare('SELECT id, tipo, categoria, subcategoria, valor, dd, mm, yyyy, status, obs, repetir, notif FROM entries WHERE user_id = ? ORDER BY yyyy, mm, dd');
$stmt->execute([$userId]);
...
if ($pushLast !== $today) {
    ...
    if ($subs) {
        ...
        if ($parts) {
            $payload = json_encode(['title' => 'TF', 'body' => implode(' ', $parts)]);
            $sent = false;
            foreach ($subs as $sub) {
